...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ==========================
        # layer = inputs.view(inputs.shape[0]*inputs.shape[1]*inputs.shape[2],inputs.shape[3])
        layer = inputs
        # print(inputs.shape)
        var = torch.var(layer, -1, unbiased=False, keepdim=True)
        # print(var.shape)
        exp = torch.mean(layer, -1, keepdim=True)
        # print(exp)
        # epsilon=1e-5
        layer = ((layer-exp) * self.weight/(torch.sqrt(var+self.eps)))  + self.bias
        return layer
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        # TODO: Write your code here
        # ==========================
        self.prj = nn.Linear(self.num_heads*self.head_size, 3*self.num_heads*self.head_size)
        self.weight_Y = nn.Linear(self.num_heads*self.head_size, self.num_heads*self.head_size) 
        # self.weight_Q = nn.Parameter(torch.Tensor(self.num_heads*self.head_size,self.num_heads*self.head_size))
        # self.bias_Q = nn.Parameter(torch.Tensor(self.num_heads*self.head_size))

        # self.weight_K = nn.Parameter(torch.Tensor(self.num_heads*self.head_size,self.num_heads*self.head_size))
        # self.bias_K = nn.Parameter(torch.Tensor(self.num_heads*self.head_size))

        # self.weight_V = nn.Parameter(torch.Tensor(self.num_heads*self.head_size,self.num_heads*self.head_size))
        # self.bias_V = nn.Parameter(torch.Tensor(self.num_heads*self.head_size))

        # self.weight_Y = nn.Parameter(torch.Tensor(self.num_heads*self.head_size,self.num_heads*self.head_size))
        # self.bias_Y = nn.Parameter(torch.Tensor(self.num_heads*self.head_size))
    ...
    def get_attention_weights(self, queries, keys):
        ...
        weight = F.softmax(torch.matmul(queries,torch.transpose(keys, -2, -1))/torch.sqrt(torch.tensor(self.head_size)), dim=-1)
        return weight
    ...
    def apply_attention(self, queries, keys, values):
        ...
        attended_values = torch.matmul(weight,values)
        attended_values = attended_values.permute(0,2,1,3)
        batch_size, sequence_length, num_heads, head_size = attended_values.shape
        attended_values = torch.reshape(attended_values, (batch_size, sequence_length, num_heads * head_size))
    ...
    def split_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        batch_size, sequence_length, dim = tensor.shape
    ...
    def forward(self, hidden_states):
        ...
        # TODO: Write your code here
        # ==========================
        # queries = torch.matmul(hidden_states ,self.weight_Q)+self.bias_Q
        # keys = torch.matmul(hidden_states ,self.weight_K)+self.bias_K
        # values = torch.matmul(hidden_states ,self.weight_V)+self.bias_V
        # print(hidden_states.shape)
        prj = self.prj(hidden_states)
        # print(prj.shape)
        queries, keys, values = torch.chunk(prj, chunks=3, dim = -1)
        # print(queries.shape, keys.shape, values.shape)
        # print(self.num_heads*self.head_size)
        Y = self.apply_attention(self.split_heads(queries), self.split_heads(keys), self.split_heads(values))
        outputs = self.weight_Y(Y)#torch.matmul(Y ,self.weight_Y)+self.bias_Y
        # print('multihead_shape',outputs.shape)
        return outputs
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.attn(out)
        out = out+x
        attention_outputs = self.layer_norm_2(out)
        out_2 =  self.linear(attention_outputs)
        res = out+out_2
        # print('prenorm',.size())
        return out+out_2
...
class VisionTransformer(nn.Module):
    ...
    def get_patches(self,image, patch_size, flatten_channels=True):
        ...
        if flatten_channels==True:
            unfold = nn.Unfold((patch_size,patch_size), dilation=1, padding=0, stride=(patch_size,patch_size))
            out = unfold(image)
            ...
            out = unfold(image)
            out = out.permute(0,2,1)
            out = out.view(B,C,int(H/patch_size*W/patch_size),patch_size,patch_size)
        return out
