...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        out, hidden = self.lstm(out, hidden_states)
        # print('lstm', out.shape)
        tag_space = self.classifier(out)
        # print('tag_space', tag_space.shape)
        log_prob = F.log_softmax(tag_space, dim=2)
    ...
    def loss(self, log_probas, targets, mask):
        ...
        vocabulary_size = log_probas.shape[2]
        # print(batch_size, sequence_length, vocabulary_size)
        loss = F.nll_loss(log_probas.view(batch_size*sequence_length,vocabulary_size), targets.reshape(-1), reduction='none')

        loss = loss.view(batch_size,sequence_length)

        loss = (loss*mask/mask.sum(1, keepdim=True)).sum()/batch_size
        return loss
